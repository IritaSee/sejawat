            ...
            <div class="row layout-top-spacing">
                <div class="col-lg-8 layout-spacing">
                    <div class="widget shadow p-3">
                        ...
                        <form action="{{ url('guru/tugas/' . $tugas->kode) }}" method="POST" enctype="multipart/form-data"
                            class="mt-3">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                        ...
                                        <select class="form-control" name="mapel" id="mapel_tugas" required>
                                            ...
                                            @foreach ($guru_mapel as $gm)
                                                @if (old('mapel', $tugas->mapel_id) == $gm->mapel->id)
                                                    <option value="{{ $gm->mapel->id }}" selected>
                                        ...
                                        <select class="form-control" name="kelas" id="kelas_tugas" required>
                                            ...
                                            @foreach ($guru_kelas as $gk)
                                                @if (old('kelas', $tugas->kelas_id) == $gk->kelas->id)
                                                    <option value="{{ $gk->kelas->id }}" selected>
                            ...
                            <div class="form-group">
                                ...
                                <div class="row">
                                    <div class="col-lg-6"><input type="date" class="form-control" name="tgl"
                                            value="{{ old('tgl', Str::substr($tugas->due_date, 0, 10)) }}" required></div>
                                    <div class="col-lg-6"><input type="time" class="form-control" name="jam"
                                            value="{{ old('jam', Str::substr($tugas->due_date, 11, 5)) }}" required></div>
                                </div>
                            </div>
                            @if ($files)
                                <p>File Sebelumnya</p>
                            ...
                            <button type="submit" class="btn btn-primary btn-sm"><span data-feather="save"></span>
                                Simpan</button>
                        </form>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            $(".summernote").summernote({
                ...
                callbacks: {
                    onImageUpload: function(e, t = this) {
                        var a;
                        e = e[0], a = t, (t = new FormData).append("image", e), $.ajax({
                            headers: {
            ...
            new FileUploadWithPreview("fileTugas");
